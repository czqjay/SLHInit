 ...
 */
function validateCallback(form, callback, addtionData) {
	var $form = $(form); 
	//if (!$form.valid()) {
	//	return false;
	//}
	$.ajax({ 
		...
		data: $.merge($form.serializeArray(),addtionData?addtionData:[]) , 
		dataType:"json",
		cache: false,
		success: function(data, successFlag,xhr){
			if(callback){
				$('#planFormAdd').find('button:contains(保存)').removeAttr('disabled');
				callback(data); 
			}else{
				DWZ.ajaxDone(xhr)
			}
		}, 
		error: function(xhr, ajaxOptions, thrownError){$('#planFormAdd').find('button:contains(保存)').removeAttr('disabled'); DWZ.ajaxError.call(null,xhr, ajaxOptions, thrownError)} 
	});
	return false;
} 
/**
