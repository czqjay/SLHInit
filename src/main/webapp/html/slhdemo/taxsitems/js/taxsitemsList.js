...
jQuery(function(){
		taxsitemsListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#taxsitemsListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addTaxsitems : function(grid, event ,closeFlag) {  //手续费税种新增
				suntoolkit.openOptions({title:'手续费税种新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/taxsitems/taxsitemsAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editTaxsitems : function(grid, rowData) { //手续费税种修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'手续费税种修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/taxsitems/taxsitemsInfoForUpdate.action" + "?id="+keyID) 
			}, 
