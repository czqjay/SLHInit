...
jQuery(function(){
		traningworkerListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#traningworkerListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addTraningworker : function(grid, event ,closeFlag) {  //工作技能学习培训意向表新增
				suntoolkit.openOptions({title:'工作技能学习培训意向表新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/traningworker/traningworkerAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editTraningworker : function(grid, rowData) { //工作技能学习培训意向表修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'工作技能学习培训意向表修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/traningworker/traningworkerInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteTraningworker : function(grid, ids) {//工作技能学习培训意向表删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/traningworker/traningworkerDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			traningworkerStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
