...
jQuery(function(){
		inputTestListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#inputTestListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addInputTest : function(grid, event ,closeFlag) {  //tb_input_test新增
				suntoolkit.openOptions({title:'tb_input_test新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/inputTest/inputTestAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editInputTest : function(grid, rowData) { //tb_input_test修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'tb_input_test修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/inputTest/inputTestInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteInputTest : function(grid, ids) {//tb_input_test删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/inputTest/inputTestDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			inputTestStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
