...
jQuery(function(){
		collecttaxListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#collecttaxListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addCollecttax : function(grid, event ,closeFlag) {  //代收税新增
				suntoolkit.openOptions({title:'代收税新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/collecttax/collecttaxAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editCollecttax : function(grid, rowData) { //代收税修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'代收税修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/collecttax/collecttaxInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteCollecttax : function(grid, ids) {//代收税删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/collecttax/collecttaxDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			collecttaxStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
