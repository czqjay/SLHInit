...
jQuery(function(){
		testListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#testListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addTest : function(grid, event ,closeFlag) {  //tb_test新增
				suntoolkit.openOptions({title:'tb_test新增',width:857,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/test/testAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editTest : function(grid, rowData) { //tb_test修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'tb_test修改',width:857,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/test/testInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteTest : function(grid, ids) {//tb_test删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/test/testDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			testStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
