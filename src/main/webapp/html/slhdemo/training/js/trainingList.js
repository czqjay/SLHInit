...
jQuery(function(){
		trainingListCRUD = { 
			...
			addTraining : function(grid, event ,closeFlag) {  //学习意向意向表新增
				suntoolkit.openOptions({title:'学习意向意向表新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/training/trainingAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editTraining : function(grid, rowData) { //学习意向意向表修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'学习意向意向表修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/training/trainingInfoForUpdate.action" + "?id="+keyID) 
			}, 
