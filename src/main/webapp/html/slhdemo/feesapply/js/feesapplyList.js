...
jQuery(function(){
		feesapplyListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#feesapplyListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addFeesapply : function(grid, event ,closeFlag) {  //手续费申请表新增
				suntoolkit.openOptions({title:'手续费申请表新增',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/slhdemo/feesapply/feesapplyAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editFeesapply : function(grid, rowData) { //手续费申请表修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'手续费申请表修改',width:868,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/feesapply/feesapplyInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteFeesapply : function(grid, ids) {//手续费申请表删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/feesapply/feesapplyDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			feesapplyStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
