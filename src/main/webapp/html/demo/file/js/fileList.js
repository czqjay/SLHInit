...
jQuery(function(){
		fileListCRUD = { 
			...
			reloadData : function(postDatas) {
				...
				jQuery("#fileListGrid").jqGrid("setGridParam", {
							postData : postDatas
						}).trigger("reloadGrid");
						   
			},  
			addFile : function(grid, event ,closeFlag) {  //tb_file新增
				suntoolkit.openOptions({title:'tb_file新增',width:857,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/html/demo/file/fileAdd.jsp?closeFlag="+closeFlag)  
			}, 
			editFile : function(grid, rowData) { //tb_file修改
				DWZ.debug(arguments) 
				var keyID = rowData.id;
				suntoolkit.openOptions({title:'tb_file修改',width:857,height:450,resizable:false,zIndex:'auto'},window.WWWROOT + "/file/fileInfoForUpdate.action" + "?id="+keyID) 
			}, 
			deleteFile : function(grid, ids) {//tb_file删除
				if (!confirm("是否要删除所选记录？(共"+ids.split(',').length+"条)")) {
					return;
				}  
				jQuery.ajax({
							type : "post", 
							url : window.WWWROOT + "/file/fileDelete.action",
							data : {
								ids : ids 
							},   
							dataType : 'json',
			...
			fileStateHbutton : function(val, cell, colpos, rwdat, _act) {
			    var str ="";
			    if(val==0){
			       str = '操作成功';
			    }else if(val==1){
